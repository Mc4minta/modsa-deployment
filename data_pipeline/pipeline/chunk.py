...
Reads:  datasets/processed/<category>/<name>.md   (output of data_pipeline.pipeline.normalize)
Writes: datasets/chunks/<category>/<name>.json    (schema เดิม — app ฝั่ง RAG ใช้ต่อได้เลย)
...
ไฟล์ใหม่ใน datasets/processed/ ที่ยังไม่มี entry ใน data_pipeline/sources.json จะได้ template
เปล่าๆ (department/source_url/contact ว่าง) auto-generate ใส่ให้ทุกครั้งที่รัน
— ไปเติมข้อมูลจริงที่นั่น ไม่ต้องแก้ในไฟล์นี้หรือใน datasets/chunks/*.json
...
OUTPUT_DIR = ROOT / "datasets" / "chunks"
SOURCES_FILE = ROOT / "data_pipeline" / "sources.json"
...
def main() -> None:
    ...
    print("ต่อไป: python -m data_pipeline.pipeline.chunk เสร็จแล้ว rebuild index (rm -rf chroma_db)")
