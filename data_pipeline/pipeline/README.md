...
แปลงเอกสารต้นฉบับ → **chunks JSON พร้อม metadata** ส่งให้ฝั่ง RAG ผ่านโฟลเดอร์ `datasets/chunks/`
...
`datasets/raw/`, `datasets/processed/`, `datasets/chunks/` ทั้ง 3 อยู่ใน `.gitignore` (กันไฟล์เอกสาร/JSON ขนาดใหญ่ขึ้น git) แปลว่า **clone repo มาใหม่จะไม่มี 3 โฟลเดอร์นี้เลย** ต้องสร้าง `datasets/raw/<หมวด>/` เองแล้วเอาไฟล์ต้นฉบับไปวาง (รับไฟล์จากเพื่อนในทีม/ไดรฟ์กลาง ไม่ใช่จาก git):

```
datasets/raw/registration/   datasets/raw/fees/            datasets/raw/academic_rules/
datasets/raw/scholarship/    datasets/raw/dormitory/       datasets/raw/academic_calendar/
datasets/raw/others/
```
...
python -m data_pipeline.pipeline.triage

# 1.5) เช็กชื่อไฟล์ใน raw/ ยาวเกิน 255 byte หรือไม่ (limit ของ filesystem)
python -m data_pipeline.pipeline.check_filenames

# 2) ต้นฉบับ -> markdown สะอาด (เรียก Typhoon ทีละหน้า เฉพาะ .pdf)
python -m data_pipeline.pipeline.normalize

# 3) markdown -> chunks JSON + metadata (อ่าน data_pipeline/sources.json)
python -m data_pipeline.pipeline.chunk
```

ผลลัพธ์: `datasets/raw/fees/x.pdf` → `datasets/processed/fees/x.md` → `datasets/chunks/fees/x.json`

### ขั้นตอน 2 — normalize: ควรรู้ก่อนรัน

- **ข้ามไฟล์ที่แปลงแล้วทั้งไฟล์**: ถ้า `datasets/processed/fees/x.md` มีอยู่แล้วจะข้ามทั้งไฟล์ (กันจ่าย Typhoon API ซ้ำ) — อยากแปลงใหม่ ให้ลบ `.md` ตัวนั้นก่อน
...
- **หน้าที่ FAILED จะหายเงียบๆ ถ้าไม่ไปแก้**: หน้านั้นจะไม่มีเนื้อหาใน chunk เลย (ไม่มี error ให้เห็นตอน chunk) ต้องเช็กเองด้วย:
  ```bash
  grep -rl "OCR FAILED" datasets/processed/
  ```
...
ปรับขนาด chunk ได้ที่หัวไฟล์ `data_pipeline/pipeline/chunk.py` (`CHUNK_SIZE` / `CHUNK_MIN` / `CHUNK_MAX`) — ค่านี้แยกจาก `CHUNK_SIZE`/`CHUNK_OVERLAP` ใน `.env` ฝั่ง RAG (อันนั้นใช้เฉพาะตอนโหลดไฟล์ดิบตรงๆ โดยไม่ผ่าน pipeline นี้)

## เติม metadata ให้ citation สวย

ทุกครั้งที่รัน `chunk`, ไฟล์ใหม่ใน `datasets/processed/` ที่ยังไม่มี entry จะถูก **auto-generate template ใส่ `data_pipeline/sources.json` ให้เอง** (title เดายังไงก็ได้จากชื่อไฟล์ department/source_url/contact ว่างไว้) พร้อม print รายชื่อไฟล์ที่เพิ่งเพิ่มให้เห็นท้ายรัน — ไม่ต้องสร้าง entry เองมือ แค่ไปแก้ **`data_pipeline/sources.json`** เติม title ให้อ่านง่าย + department/source_url/contact ต่อไฟล์ แล้วรัน `python -m data_pipeline.pipeline.chunk` ใหม่อีกรอบ
— อย่าแก้ใน `datasets/chunks/*.json` เพราะถูก generate ทับทุกครั้ง

ถ้าไฟล์ใน `datasets/processed/` ถูกลบ/เปลี่ยนชื่อ entry เก่าใน `sources.json` จะไม่ถูกลบอัตโนมัติ (กันข้อมูลที่กรอกไว้หาย) แต่ `chunk` จะเตือน `⚠️ data_pipeline/sources.json มี N entry ที่ไม่มีไฟล์ processed แล้ว` ท้ายรัน — เช็กแล้วค่อยลบ/ย้าย entry นั้นไปไว้ที่ doc_id ใหม่เอง

## หลังรันเสร็จ

1. เช็กหน้า OCR ที่ยังไม่ผ่าน: `grep -rl "OCR FAILED" datasets/processed/` แล้วไปแก้ให้ครบก่อน chunk รอบสุดท้าย
2. 🔴 **ตรวจของเสี่ยง** (เงิน/วันที่/เกณฑ์ทุน) ใน `datasets/processed/` กับต้นฉบับ — OCR/extract อาจผิดเงียบๆ
3. ⚠️ **วรรณยุกต์ที่หาย** (เช่น `คาใชจาย` → `ค่าใช้จ่าย`) `clean` ซ่อมไม่ได้ ต้องแก้มือใน `datasets/processed/`
4. ฝั่ง RAG อ่าน `datasets/chunks/` อยู่แล้ว (`.env`: `RAG_SOURCE_PATHS="../datasets/chunks"`) → รีสตาร์ทแอปเพื่อ re-index (หรือยิง `POST /admin/reindex`)

## หมายเหตุ

- จุดส่งต่อระหว่าง 2 ฝั่งคือ **`datasets/chunks/` เท่านั้น** (ฝั่ง RAG ไม่ยุ่งกับ `datasets/raw`/`processed`)
- `datasets/raw/`, `datasets/processed/` **ไม่ถูก commit**; `datasets/chunks/` ถูก commit เป็น handoff ให้ฝั่ง RAG ใช้ตรง — แชร์ไฟล์ต้นฉบับกับเพื่อนนอก git แล้วให้แต่ละคนรัน pipeline เอง ผลลัพธ์ (`datasets/processed/`) จะไม่ตรงกันเป๊ะถ้าคนละเวอร์ชัน OCR — ถ้าจะแชร์ผลลัพธ์กันจริง commit `datasets/chunks/` ตรงๆ ไปเลยจะชัวร์กว่า
